import { cliVersion, WEB3SDKS_URL } from "../constants/urls";
...
export async function processProject(
  ...
  if (projectType === "none") {
    if (command === "deploy") {
      info(
        "No contracts detected in this directory. Redirecting to the web3sdks contract deployment page.",
      );
      return "https://web3sdks.com/contracts";
    }
    ...
    logger.info(``);
    logger.info(`     ${chalk.cyan(`npx web3sdks@latest create --contract`)}`);
    logger.info(``);
    process.exit(1);
  }
  ...
  const soliditySDKPackage = "@web3sdks/contracts";
...
export function getUrl(hashes: string[], command: string) {
  ...
  if (hashes.length === 1) {
    ...
    url = new URL(WEB3SDKS_URL + "/contracts/" + command);
