...
const main = async () => {
  ...
  const skipIntro = process.env.WEB3SDKS_CLI_SKIP_INTRO === "true";
...
\\_____\\____/  \\_______| \\_______/  \\_______/ \\______/   \\______/  \\__/ \\__/ \\______/ `);
    console.info(`\n 💎 web3sdks-cli v${cliVersion} 💎\n`);
  }

  program
    .name("web3sdks-cli")
    .description("Official web3sdks command line interface")
    .version(cliVersion, "-v, --version");

  program
    .command("create [projectType] [projectPath]")
    .description(
      "Create a web3 app from any of our official templates: https://github.com/web3sdks-template/",
    )
    ...
    .option(
      "-t, --template [name]",
      "A template to start your project from. You can use an template repository name from the official web3sdks-template org.",
    )
    .option(
  ...
  program
    .command("build")
    .description("Compile contract and detect web3sdks contract extensions")
  ...
  program
    .command("detect")
    .description(
      "(deprecated) Compile contracts and detect implemented web3sdks contract extensions",
    )
    .option("-p, --path <project-path>", "path to project", ".")
    .option("-d, --debug", "show debug logs")
    .option("-a, --all", "run detection on all contracts")
      ...
      async ({ default: updateNotifier }) => {
        ...
        if (versionInfo.type !== "latest") {
          ...
          if (res.upgrade) {
            ...
            clonedEnvironment.WEB3SDKS_CLI_SKIP_INTRO = "true";

            const installation = await findPackageInstallation();

            // If the package isn't installed anywhere, just defer to npx web3sdks@latest
            if (!installation) {
              ...
              await new Promise((done, failed) => {
                const shell = spawn(
                  `npx --yes web3sdks@latest ${process.argv
                    .slice(2)
                    .join(" ")}`,
                  [],
            ...
            switch (installation.packageManager) {
              case "npm":
                command = installation.isGlobal
                  ? `npm install -g web3sdks`
                  : `npm install web3sdks`;
                break;
              case "yarn":
                command = installation.isGlobal
                  ? `yarn global add web3sdks`
                  : `yarn add web3sdks`;
                break;
              case "pnpm":
                command = installation.isGlobal
                  ? `pnpm add -g web3sdks@latest`
                  : `pnpm add web3sdks@latest`;
                break;
              default:
                console.error(
