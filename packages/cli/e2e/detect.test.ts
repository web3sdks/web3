...
describe("npx web3sdks detect", () => {
  it("should detect ERC721Base extensions", async () => {
    ...
    expect(await exists("web3sdks-contracts/contracts/Contract.sol")).toEqual(
      true,
    );

    // check that hardhat config exists
    expect(await exists("web3sdks-contracts/hardhat.config.js")).toEqual(true);

    await create.writeText("cd web3sdks-contracts");
    await create.pressKey("enter");
