...
export const mochaHooks = {
  beforeAll: async () => {
    ...
    const web3sdksFactoryDeployer = (await new ethers.ContractFactory(
    ...
    const deployTxFactory = web3sdksFactoryDeployer.deployTransaction;
    await deployTxFactory.wait();
    const web3sdksRegistryAddress = await web3sdksFactoryDeployer.registry();
    registryAddress = web3sdksFactoryDeployer.address;

    await registryContract.grantRole(
      await registryContract.OPERATOR_ROLE(),
      web3sdksFactoryDeployer.address,
    );
    ...
    for (const contractType in CONTRACTS_MAP) {
      ...
      for (const factory of factories) {
        ...
        const tx = await web3sdksFactoryDeployer.addImplementation(
    ...
    process.env.registryAddress = web3sdksRegistryAddress;
    // eslint-disable-next-line turbo/no-undeclared-env-vars
    process.env.factoryAddress = web3sdksFactoryDeployer.address;
    // eslint-disable-next-line turbo/no-undeclared-env-vars
