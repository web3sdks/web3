...
import DeprecatedAbi from "@web3sdks/contracts-js/dist/abis/IDelayedRevealDeprecated.json";
import { TokensLazyMintedEvent } from "@web3sdks/contracts-js/dist/declarations/src/DropERC721";
...
> {
  ...
  private async isLegacyContract(): Promise<boolean> {
    if (
      hasFunction<IWeb3sdksContract>("contractVersion", this.contractWrapper)
    ) {
      try {
        const version =
