...
export class ContractDeployer extends RPCConnectionHandler {
  ...
  >(
    ...
    return this.deployReleasedContract(
      // 0x31ffaf93c7de25019fd09419e252c711343d8393 === deployer.web3sdks.eth
  ...
  public async deployReleasedContract(
    ...
    const release = await new Web3sdksSDK(process.env.NEXT_PUBLIC_RELEASE_NETWORK || "polygon")
      .getPublisher()
  ...
  public async deployProxy(
    ...
    const { TWProxy__factory } = await import(
      "@web3sdks/contracts-js/factories/TWProxy__factory"
    );
