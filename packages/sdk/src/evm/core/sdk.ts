 ...
 * @public
 */
export class Web3sdksSDK extends RPCConnectionHandler {
  /**
  ...
  static async fromWallet(
    ...
    return Web3sdksSDK.fromSigner(signer, network, options, storage);
  }
  ...
  static fromSigner(
    ...
    const sdk = new Web3sdksSDK(network || signer, options, storage);
  ...
  /**
   * @internal
   * should never be accessed directly, use {@link Web3sdksSDK.getPublisher} instead
   */
  private _publisher: ContractPublisher;
  /**
   * Internal handler for uploading and downloading files
   */
  private storageHandler: Web3sdksStorage;
  /**
   ...
   */
  public storage: Web3sdksStorage;
  /**
  ...
  public async getContractList(walletAddress: string) {
    ...
    return addressesWithContractTypes
      ...
      .map(({ address, contractType, metadata }) => {
        invariant(metadata, "All Web3sdksContracts require metadata");
        return {
          address,
          contractType,
