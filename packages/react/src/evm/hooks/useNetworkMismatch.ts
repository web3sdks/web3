 ...
 * ```
 *
 * @returns `true` if the chainId of the connected wallet is different from the desired chainId passed into <Web3sdksProvider />
 *
 * @example
...
export function useNetworkMismatch() {
  ...
  if (desiredChainId === -1) {
    // means no desiredChainId is set in the <Web3sdksProvider />, so we don't care about the network mismatch
    return false;
  }
  if (!walletChainId) {
